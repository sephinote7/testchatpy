...
SYSTEM_PROMPT_BASE = """당신은 공감적이고 차분한 고민 상담 AI 상담사입니다.
...
def _build_system_prompt(mbti: str | None = None, persona: str | None = None) -> str:
    prompt = SYSTEM_PROMPT_BASE
    extra = []
    if mbti:
        extra.append(f"- 사용자 MBTI: {mbti}. 성향을 참고해 소통 톤을 맞추세요.")
    if persona:
        extra.append(f"- 사용자(persona/프로필): {persona}. 상담 시 참고하세요.")
    if extra:
        prompt += "\n4) 사용자 참고 정보 (있는 경우에만 반영)\n" + "\n".join(extra)
    return prompt


def get_ai_reply(user_text: str, history: list, mbti: str | None = None, persona: str | None = None) -> str:
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        return "죄송합니다. AI 상담이 일시적으로 이용 불가합니다."
    client = OpenAI(api_key=api_key)
    system_prompt = _build_system_prompt(mbti, persona)
